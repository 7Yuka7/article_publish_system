  ...
  <div class="row">
    <div v-for="colum in columnList" :key="colum.id" class="col-4 mb-4">
      <div class="card h-100 shadow-sm">
        <img :src="colum.avatar" :alt="colum.title" class="rounded-circle mx-auto border border-light w-25 my-3">
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const columnList = computed(() => {
      return props.list.map(colum => {
        // 如果没有avatar项就会补充
        if (!colum.avatar) {
          colum.avatar = require('@/assets/logo.png')
        }
        return colum
      })
    })
